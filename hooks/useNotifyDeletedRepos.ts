...
export function useNotifyDeletedRepos(
  ...
  // On stocke ici la liste des dépôts qu'on a DÉJÀ notifiés
  ...
  useEffect(() => {
    // Si pas de dépôts supprimés, on ne fait rien
    if (!reposDeletedFromGithub?.length) return;

    // On filtre pour garder SEULEMENT les dépôts qu'on n'a pas encore notifiés
    const newRepos = reposDeletedFromGithub.filter(
      (repo) => !notifiedReposRef.current.has(repo)
    );

    // Si tous les dépôts ont déjà été notifiés, on ne fait rien
    if (newRepos.length === 0) return;

    // On marque ces nouveaux dépôts comme "déjà notifiés" pour éviter les doublons
